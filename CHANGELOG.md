...
- Per-rule suppression: `--ignore RULE_ID` (repeatable, supports a bare family prefix like `system-prompt`) on both `check` and `fix`, plus a `.llm-prompt-lint.json` sidecar config file (`{"ignore": [...]}`, `--config` to point elsewhere) so a team can check a suppression in rather than repeat the flag. Deliberately not embedded in the request JSON itself, since those files are meant to double as real API request bodies.
